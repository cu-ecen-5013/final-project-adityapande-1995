...
long long UF_timestamps[RECEIVE_BUFFER_SIZE*2] = {0};
int UFB_index = 1;
...
void generate_receiver_buffer(){
  ...
  for(int i = 1 ; i < UFB_index; i++){
    /*  Rising edge */
    if(UF_buffer[i] == 1){
      fraction = (UF_timestamps[i+1] - UF_timestamps[i])/CLOCK_MS;
      if (fraction > 0){
...
int main(int argc, char *argv[]){
  ...
  gpioSetAlertFunc(MSG_IN_PIN, msg_in_callback);
  ...
  /* Print results */
  //for(int i = 1; i < UFB_index; i++)printf("\n Edge %d at timestamp %llu, diff with prev: %llu", UF_buffer[i], UF_timestamps[i], UF_timestamps[i] - UF_timestamps[i-1]);
